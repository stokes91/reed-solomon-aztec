...
ReedSolomonEncoder.factory = (field, BLOCKS_ECC) => {
  ...
  for (let i = 0; i < BLOCKS_ECC; i++) {
    for (let j = 0; j < BLOCKS_ECC; j++) {
      that.coefficients[j] = field.Multiply(that.coefficients[j], root);
      if (j + 1 < BLOCKS_ECC)
        that.coefficients[j] = field.Add(that.coefficients[j], that.coefficients[j + 1]);
    }
