...
function ReedSolomonDecoder(field, array, BLOCKS_ECC, correctArray) {
  const rsSyndrome = GaloisFieldPolynomial.from(field, array, BLOCKS_ECC);
  if (rsSyndrome.degree() === 0 && rsSyndrome.leadingCoefficient() === 0) return false;
  ...
  return that.repair(array, correctArray);
}
